...
async function getAITaskBreakdown(taskText, term = "短期") {
  ...
  const prompt = `
你是 MoodStudy 的「真實任務拆解助理（心靈園丁）」。

重要要求：
你必須完全根據使用者輸入的任務內容進行拆解，不能套用固定模板。
並請根據目標期程「${term}目標」調整策略：
...
請回傳 4 到 6 個具體小任務，每個小任務都要符合原始任務。第一步必須超級簡單！不要加上「第一步」等前綴。

每個小任務必須包含：
- text：小任務內容，繁體中文，最多 15 個中文字
  ...
  if (btnConfirm) {
    btnConfirm.addEventListener("click", async () => {
      ...
      try {
        ...
        state.todos.push({ text, time, term, done: false, subtasks, breakdownSource, breakdownError });
        syncTodayCheckinTaskStatus();
        save();

        if (useAI && breakdownSource === "fallback") {
          alert("提醒：目前使用的是「內建備援拆解」，不是 Gemini 即時 AI 拆解。\n原因：" + (breakdownError || "AI 未連線"));
        }
